...
[`airway` R package](https://bioconductor.org/packages/release/data/experiment/vignettes/airway/inst/doc/airway.html). 
Very briefly:
...
```{r, message = FALSE}
# Clean your workspace by removing any objects created before
rm(list = ls())

# Load the tidyverse package
library(tidyverse)

# Load the RNAseq data provided by your collaborator
...
### Calculate the average gene expression per treatment

To do this, we need a few steps:

1. Convert table to "long" format, i.e. with 3 columns:
    * `gene`
    * `sample_id`
    * `cts`
2. Join this table with the `sample_info` table 
3. Filter "significant" genes from `test_result` table and classify them as up- or 
down-regulated. Join this table to the previous one.
4. Use "split-apply-combine" approach to summarise data per treatment and sample

#### Step 1: reshaping data to "long" format

There's two functions that allow us to convert tables from a "wide" to a "long" 
format and vice-versa: `gather()` and `spread()`

[!spread](http://www.datacarpentry.org/R-ecology-lesson/img/spread_data_R.png)

[!gather](http://www.datacarpentry.org/R-ecology-lesson/img/gather_data_R.png)
...
#### Step 2: join two tables together
...
#### Step 3: Filter and classify genes
...
#### Step 4: Calculate mean expression per gene

Now, we can apply the "split-apply-combine" strategy that we've already learned 
about, to summarise the data per gene.
...
With this table, we are now ready to produce our plot!
...
cts_filtered %>% 
  ...
  scale_fill_gradient2(low = "royalblue", mid = "grey", high = "red3") +
